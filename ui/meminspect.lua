...
local function getObjAddr(obj)
	...
	try(function()
		...
		result = memedit.dll.debug.getObjAddr(obj)
	end)
	:catch(function()
...
local function isAddrNotPtr(addr, size)
	...
	try(function()
		...
		result = memedit.dll.debug.isAddrNotPointer(addr, size)
	end)
	:catch(function()
...
local function getAddrInt(addr)
	...
	try(function()
	Assert.Equals('number', type(addr))
		result = memedit.dll.debug.getAddrInt(addr)
	end)
	:catch(function()
...
local function getAddrValue(addr)
	...
	try(function()
	Assert.Equals('number', type(addr))
		result = memedit.dll.debug.getAddrValue(addr)
	end)
	:catch(function()
...
local function calculateOpenButtonPosition()
	-- Place button in the bottom right corner
	return
		screen:w() - BUTTON.MAIN.WIDTH - 14,
		screen:h() - BUTTON.MAIN.HEIGHT - 14
end
